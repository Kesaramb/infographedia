...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  // ── Comments table (may already exist from manual Fix 1) ──────────────
  await db.execute(sql`
  ...
  await db.execute(sql`
    ...
    CREATE TABLE IF NOT EXISTS "ai_agent_config_allowed_chart_types" (
    ...
    CREATE TABLE IF NOT EXISTS "ai_agent_config_allowed_themes" (
      "order" integer NOT NULL,
      "parent_id" integer NOT NULL,
    ...
    CREATE TABLE IF NOT EXISTS "ai_agent_config_few_shot_examples" (
  ...
  await db.execute(sql`
    ...
    ALTER TABLE "posts" ADD COLUMN IF NOT EXISTS "metrics_comments" numeric DEFAULT 0;
    ALTER TABLE "payload_locked_documents_rels" ADD COLUMN IF NOT EXISTS "comments_id" integer;
  `)

  // ── Foreign keys (safe: check before adding) ──────────────────────────
  await db.execute(sql`
    DO $$ BEGIN
  ...
  await db.execute(sql`
    ...
    CREATE INDEX IF NOT EXISTS "ai_agent_config_allowed_chart_types_order_idx" ON "ai_agent_config_allowed_chart_types" USING btree ("order");
    CREATE INDEX IF NOT EXISTS "ai_agent_config_allowed_chart_types_parent_idx" ON "ai_agent_config_allowed_chart_types" USING btree ("parent_id");
    CREATE INDEX IF NOT EXISTS "ai_agent_config_allowed_themes_order_idx" ON "ai_agent_config_allowed_themes" USING btree ("order");
    CREATE INDEX IF NOT EXISTS "ai_agent_config_allowed_themes_parent_idx" ON "ai_agent_config_allowed_themes" USING btree ("parent_id");
    CREATE INDEX IF NOT EXISTS "ai_agent_config_few_shot_examples_order_idx" ON "ai_agent_config_few_shot_examples" USING btree ("_order");
    CREATE INDEX IF NOT EXISTS "ai_agent_config_few_shot_examples_parent_id_idx" ON "ai_agent_config_few_shot_examples" USING btree ("_parent_id");
